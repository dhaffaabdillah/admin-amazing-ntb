...
class _CitiesPageState extends State<States> {
    ...
    return Column(
      ...
      children: [
        ...
        Expanded(
          ...
          ? EmptyPage(icon: Icons.content_paste, message: 'No states found.\nUpload states first!')
          : RefreshIndicator(
  ...
  Future _updateState (String stateTimestamp) async{
    final DocumentReference ref = firestore.collection(collectionName).doc(stateTimestamp);
    await ref.update({
      'name' : nameCtrl1.text,
  ...
  void openEditDialog (String oldStateName, String oldThumbnailUrl, String stateTimestamp){
    showDialog(
      context: context,
      builder: (context){
        ...
        return SimpleDialog(
            contentPadding: EdgeInsets.all(100),
            children: <Widget>[
              Text('Edit/Update the State to Database', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900),),
              SizedBox(height: 50,),
              Form(
                key: formKey1,
                child: Column(children: <Widget>[
                  TextFormField(
                  decoration: inputDecoration('Enter State Name', 'State Name', nameCtrl1),
                ...
                onPressed: ()async => _handleUpdateState(stateTimestamp),
              ),

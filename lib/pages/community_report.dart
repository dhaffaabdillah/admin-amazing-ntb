...
import 'package:admin/pages/update_community_report.dart';
...
class CommunityReportPage extends StatefulWidget {
  const CommunityReportPage({Key? key}) : super(key: key);

  @override
  _CommunityReportPageState createState() => _CommunityReportPageState();
}
...
class _CommunityReportPageState extends State<CommunityReportPage> {
    ...
    if (data.docs.length > 0) {
      ...
      if (mounted) {
        setState(() {
          ...
          _data = _snap.map((e) => CommunityReportModel.fromFirestore(e)).toList();
        });
      }
    } else {
      if (_lastVisible == null) {
  ...
  Widget dataList(CommunityReportModel d) {
    return Container(
                      ...
                      InkWell(
                        child: Container(
                            ...
                            child: Icon(Icons.edit,
                                size: 16, color: Colors.grey[800])),
                        onTap: () {
                          nextScreen(context, UpdateCommunityReport(CommunityReportData: d));
                        },
                      ),
                    ],
                  ),
